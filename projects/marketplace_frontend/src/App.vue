      ...
      <div class="items">
        <div class="row">
          <div
            ...
            v-for="item in items"
            v-bind:key="item.idItem"
          >
            <div class="card card-body text-center">
              <h5>{{ item.description }}</h5>
              <h6>{{ formatMoney(item.price) }}</h6>
              <br />
              <button class="btn btn-info btn-sm" @click="addItem(item)">
        ...
        <div v-if="!newOrder.code">
            ...
            <div class="row">
              ...
              <div class="col-xs-2 col-sm-2 col-md-2 text-right">
                <button
                  ...
                  @click="deleteItem(orderItem.idItem)"
                >
          ...
          <div class="form-group">
            ...
            <input
              ...
              @blur="validateCoupon(order)"
            />
          </div>
          <hr />
          <div class="text-right">
            <div>Subtotal: {{ formatMoney(subtotal) }}</div>
            <div>Desconto: {{ formatMoney(discount) }}</div>
            <div>Frete: {{ formatMoney(freight) }}</div>
            <div>Total: {{ formatMoney(total) }}</div>
          </div>
          <hr />
          <button class="btn btn-info btn-block" @click="confirm(order)">
            Confirmar
          </button>
        </div>
      </div>
